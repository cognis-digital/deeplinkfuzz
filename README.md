...
[![PyPI](https://img.shields.io/pypi/v/cognis-deeplinkfuzz.svg)](https://pypi.org/project/cognis-deeplinkfuzz/)
[![CI](https://github.com/cognis-digital/deeplinkfuzz/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/deeplinkfuzz/actions)
...
DEEPLINKFUZZ exists for one job — fuzzes android/ios deep links, intents, and custom url schemes against an emulator/device to surface unvalidated-redirect, injection, and component-hijack bugs. — and does it without a SaaS bill or heavyweight setup.
...
deeplinkfuzz scan .                      # scan the current project
deeplinkfuzz scan . --format json
deeplinkfuzz scan . --fail-on high       # non-zero exit for CI gates
deeplinkfuzz mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **Drozer (Android attack surface) + scheme-fuzzing techniques from mobile pentest playbooks**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`deeplinkfuzz` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
