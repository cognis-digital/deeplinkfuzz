"""Native cognis-connect emit for deeplinkfuzz — forward findings to any platform.

Maps deeplinkfuzz's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    deeplinkfuzz ... --format json | deeplinkfuzz-emit --to stix
    deeplinkfuzz-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('component') or rec.get('url')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'injection')
